...
API_ID = 0                             # ← свой api_id с my.telegram.org
API_HASH = "ВСТАВЬ_API_HASH"           # ← свой api_hash с my.telegram.org

# ─── СЕССИИ ДЛЯ ПУЛА (2 шт.) ──────────────────────────────────────────────────
POOL_SESSIONS = [
    ("parsersess1", "+00000000000"),   # ← свой номер
    ("parsersess2", "+00000000000"),   # ← свой номер
]
...
FILTER_MIN_GIFTS = 1
FILTER_MAX_GIFTS = 6
SEARCH_TIMEOUT = 180.0     # верхняя граница одного поиска, сек
# Сколько задач держим «в полёте». Реальных слотов у пула всего
# len(POOL_SESSIONS) * POOL_CONCURRENCY_PER_SESSION (= 4), поэтому 40 задач
...
# оставлена только для совместимости/диагностики — ждать на ней нельзя,
# иначе флуд одной сессии усыпляет все задачи разом.
...
async def generate_random(limit: int, tier: str | None = None,
    ...
    на её место сразу встаёт новая. Поэтому поиск не выдыхается после первой
    волны, а добирает ровно до limit или до SEARCH_TIMEOUT.
    ...
    def make_owner_task():
        n, i = random_entry(tier)
        return asyncio.create_task(get_nft_owner(n, i))

    async def render():
        nonlocal last_text, last_edit
        if message is None or time.monotonic() - last_edit < EDIT_INTERVAL:
            return
        text = f"<b>{label}</b>\n⏳ Ищу... найдено <b>{len(found)}/{limit}</b>\n\n"
        for fr in found:
            profile = (f"@{fr['username']}" if fr.get("username")
                       else f"<a href='tg://user?id={fr['user_id']}'>профиль</a>")
            text += (f"<a href='{fr['link']}'>{fr['nft_name']} #{fr['nft_id']}</a> — "
                     f"{profile} · {profile_line(fr)}\n\n")
    ...
    pending: set = set()
    enrich_pending: set = set()

    while len(found) < limit and time.monotonic() < deadline:
        # пополняем пул на каждой итерации — так провалившиеся задачи
        # всегда заменяются новыми, и поиск не останавливается на 1-2 из 20
        while len(pending) + len(enrich_pending) < SEARCH_CONCURRENCY:
        ...
        for task in done_set:
            ...
            if getattr(task, "_is_enrich", False):
                if res and len(found) < limit:
                    # ключи name/id — для совместимости с рендером хэндлеров
    ...
    print(f"[random] финиш: {len(found)}/{limit} за "
          f"{SEARCH_TIMEOUT - max(0.0, deadline - time.monotonic()):.0f}с")
    return found[:limit]
...
async def get_nft_owner(name: str, nft_id: int) -> dict | None:
    ...
    for attempt in range(MAX_RETRIES):
        # Глобального ожидания нет: переключением сессий при FloodWait
        # занимается сам пул (SessionProxy._run).
        try:
            ...
            # Пул уже перебрал сессии и всё равно поймал FloodWait — не спим
            # глобально, просто отдаём None: стриминговый цикл поставит
            # на освободившийся слот новую задачу.
            _flood_until = time.monotonic() + e.value + 1
            print(f"[get_nft_owner] FloodWait {e.value}s on {name}-{nft_id} — пропускаю")
            return None
        except Exception as e:
            s = str(e)
...
async def get_user_profile(user_id: int) -> dict | None:
    ...
    if not pyro_client:
        return None
    # Без глобального ожидания: resolve_peer + GetFullUser + GetSavedStarGifts
    # идут одним блоком под _PYRO_SEM, флуд разруливает пул.
    try:
        async with _PYRO_SEM:
...
async def cb_mode_select(callback: CallbackQuery):
    ...
    label = TIER_LABELS.get(tier, "🎲 Рандом")
    await callback.message.edit_text(
        f"<b>{label}</b>\n⏳ Ищу... найдено <b>0/{limit}</b>", parse_mode="HTML")
    results = await generate_random(limit, tier=tier, message=callback.message)
    if not results:
        await callback.message.edit_text(
            "❌ Никого не нашёл — попробуй ещё раз",
            reply_markup=again_kb(), parse_mode="HTML")
        return
    text = f"<b>{label}</b> | найдено: {len(results)}/{limit}\n\n"
    for item in results:
        profile = (f"@{item['username']}" if item.get("username")
                   else f"<a href='tg://user?id={item['user_id']}'>профиль</a>" if item.get("user_id")
                   else "—")
        text += (f"<a href='{item['link']}'>{item['name']} #{item['id']}</a> — "
                 f"{profile} · {profile_line(item)}\n\n")
    await callback.message.edit_text(text, reply_markup=again_kb(), disable_web_page_preview=True, parse_mode="HTML")
